...
const questions = [{
    ...
    validate: name => name.includes(".") ? "Project names cannot contain the dot ('.') character" : true
}, {
    type: "select",
    name: "variant",
...
const dir = parseResults._.filter(f => typeof f === "string").length === 0 ? "." : parseResults._.filter(f => typeof f === "string")[0]
