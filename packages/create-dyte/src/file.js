...
export class File {
     ...
     */
    async create(at) {
        await Deno.create(join(at ?? Deno.cwd(), this.name)).then(async file => await file.write(new TextEncoder().encode(this.contents)));
    }
}
...
export class Folder {
    ...
    async create(at, force) {
        ...
        try {
            ...
            throw new ExistsError(`Directory at ${folderDir} already exists`, {cause: e});
        }
