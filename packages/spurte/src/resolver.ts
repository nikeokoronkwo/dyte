  ...
  jsr() {
    this.code = this.code.replaceAll(
      ...
      (match, scope, name, version: string = "", path = ""): string => {
        const url = Array.from(this.sourceMap.keys()).find((e) =>
          validate(e, match)
        );
        if (!url) {
          console.error(
            "The given JSR URL could not be resolved in the source map",
          );
          throw new Error(
            "The given JSR URL could not be resolved in the source map",
          );
...
function validate(url: string, importPath: string): boolean {
  ...
  if (
    packageParts.slice(0, 2).every((pp, index) =>
      urlParts.slice(0, 2)[index] == pp
    )
  ) {
    if (packageParts.length >= 4) {
      if (packageParts.every((pp, index) => urlParts[index] == pp)) return true;
    } else if (
      packageParts.slice(2).every((pp, index) => urlParts.slice(3)[index] == pp)
    ) {
      return true;
    } else {
      if (pkgInfo.endsWith("mod.ts") && packageParts[2] == urlParts[2]) {
        return true;
      }
    }
  }
